    ...
    pub fn build(plan: &ExecutionPlan) -> Result<ExecutionManifest, BuilderError> {
        ...
        for seg in &plan.segments {
            let instruction = match seg.instruction {
                ExecutionInstruction::MoveJ => ManifestInstruction::MoveJ,
                ExecutionInstruction::MoveL => ManifestInstruction::MoveL,
            };
...
mod tests {
    ...
    fn two_segment_manifest() -> ExecutionManifest {
        let p = plan(
            ...
            vec![
                seg(0, 0, ExecutionInstruction::MoveJ, 0..2),
                seg(1, 1, ExecutionInstruction::MoveL, 2..3),
            ],
            1.0,
        );
    ...
    fn manifest_segment_uses_planned_segment_provenance() {
        ...
        let p = plan(
            ...
            vec![
                seg(0, 0, ExecutionInstruction::MoveJ, 0..2),
                seg(1, 1, ExecutionInstruction::MoveL, 2..4),
                seg(2, 2, ExecutionInstruction::MoveJ, 4..5),
            ],
            1.5,
        );
    ...
    fn dof_mismatch_rejected() {
        let p = plan(
            ...
            vec![seg(0, 0, ExecutionInstruction::MoveJ, 0..2)],
            0.5,
        );
        let err = ExecutionManifestBuilder::build(&p).expect_err("build must fail");
        assert!(matches!(
