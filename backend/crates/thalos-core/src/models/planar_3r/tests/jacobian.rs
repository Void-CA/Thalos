...
fn at_zero_configuration() {
    ...
    assert!(
        dx_dq1.abs() < 1e-6,
        "dx/dθ1 should be 0, got {}",
        dx_dq1
    );
    assert!(
        dx_dq2.abs() < 1e-6,
        "dx/dθ2 should be 0, got {}",
        dx_dq2
    );
    assert!(
        dx_dq3.abs() < 1e-6,
        "dx/dθ3 should be 0, got {}",
        dx_dq3
    );
    assert!(
        (dy_dq1 - 3.0).abs() < 1e-4,
    ...
    assert!(
        dz_dq1.abs() < 1e-6,
        "dz/dθ1 should be 0 for planar robot"
    );
    assert!(
        dz_dq2.abs() < 1e-6,
        "dz/dθ2 should be 0 for planar robot"
    );
    assert!(
        dz_dq3.abs() < 1e-6,
        "dz/dθ3 should be 0 for planar robot"
    );
}
...
fn third_joint_effect() {
    ...
    let p_original = fk.evaluate(&q)
        .pose(&end_effector)
        .unwrap()
        .transform()
        .translation;
    
    let p_perturbed = fk.evaluate(&q_perturbed)
        .pose(&end_effector)
        .unwrap()
        .transform()
        .translation;
