...
fn geometric_jacobian_propagates_velocities() {
    ...
    let fk =
        ForwardKinematics::new(
            robot.clone()
        );

    let jacobian =
        GeometricJacobian::new(
            fk,
            end_effector,
        );
    ...
    let v_x =
        j.linear[(0,0)] * q_dot[0]
        +
        j.linear[(0,1)] * q_dot[1];

    let v_y =
        j.linear[(1,0)] * q_dot[0]
        +
        j.linear[(1,1)] * q_dot[1];

    let omega_z =
        j.angular[(2,0)] * q_dot[0]
        +
    ...
    let fk_solver =
        ForwardKinematics::new(robot);

    let current =
        fk_solver.evaluate(&q);

    let next =
        fk_solver.evaluate(&q_next);
    ...
    let v_actual_x =
        (p_next.x - p_current.x) / dt;

    let v_actual_y =
        (p_next.y - p_current.y) / dt;

    assert!(
        (v_x - v_actual_x).abs() < 1e-4
    );

    assert!(
        (v_y - v_actual_y).abs() < 1e-4
    );
