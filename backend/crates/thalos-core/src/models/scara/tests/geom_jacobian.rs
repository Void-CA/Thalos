...
fn prismatic_joint_only_affects_z() {
    ...
    for q in test_configs {
        ...
        assert!(j.angular[(1, 2)].abs() < 1e-6, "Prismatic ωy should be 0 at q={:?}", q);
        assert!(j.angular[(2, 2)].abs() < 1e-6, "Prismatic ωz should be 0 at q={:?}", q);
    }
}
...
fn wrist_joint_does_not_affect_position() {
    ...
    for q in test_configs {
        ...
        assert!(j.linear[(0, 3)].abs() < 1e-6, "Wrist should not affect X at q={:?}", q);
...
fn linearity() {
    ...
    let linear_combined = (a * jv1.0 + b * jv2.0, a * jv1.1 + b * jv2.1, a * jv1.2 + b * jv2.2);

    assert!((combined.0 - linear_combined.0).abs() < 1e-12, "Linearity fails in X");
    assert!((combined.1 - linear_combined.1).abs() < 1e-12, "Linearity fails in Y");
    assert!((combined.2 - linear_combined.2).abs() < 1e-12, "Linearity fails in Z");
}
