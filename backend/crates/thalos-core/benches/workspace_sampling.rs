...
fn sample_scara_10k(c: &mut Criterion) {
    ...
    c.bench_function("sample/scara_10k", |b| {
        b.iter(|| {
            ...
            sampler
                .sample(black_box(&chain), black_box(config), &mut rng)
        })
    });
}
...
fn sample_planar2r_10k(c: &mut Criterion) {
    let chain =
        RobotRegistry::create(RobotModel::Planar2R, RobotModel::Planar2R.default_spec())
...
criterion_group!(sampling, sample_scara_10k, sample_scara_100k, sample_planar2r_10k);
