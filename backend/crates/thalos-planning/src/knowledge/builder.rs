...
impl Default for WorkspaceSamplingConfig {
    fn default() -> Self {
        Self { coarse_samples: 10_000, refinement_depth: 0, max_samples: 100_000, seed: 42 }
    }
}
...
impl MonteCarloBuilder {
    ...
    pub fn build(&self, chain: &Arc<SerialChain>) -> WorkspaceKnowledge {
        ...
        let sample_count = self.config.coarse_samples
        ...
        let workspace = sampler.sample::<StdRng>(
            chain,
            ws_config,
            &mut rng,
        ...
        let reachability = Some(ReachabilityMap {
            samples: workspace.samples().iter().map(|s| ReachabilitySample {
                position: s.position,
                reachable: true,
            }).collect(),
        });
        ...
        let manip_samples: Vec<ManipulabilitySample> = workspace.samples().iter().map(|s| {
        ...
        let manipulability = if !manip_samples.is_empty() {
            Some(ManipulabilityField { samples: manip_samples })
        } else {
            None
        };
    ...
    fn preferred_configs(&self, workspace: &Workspace) -> Vec<ConfigurationRegion> {
        ...
        let mut configs: Vec<(usize, f64)> = workspace.samples().iter().enumerate().map(|(i, s)| {
        ...
        configs[..top_n.min(configs.len())].iter().map(|(idx, dist)| {
