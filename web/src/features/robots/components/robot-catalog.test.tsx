 ...
 * the catalog shows the error message AND a "Reintentar" button, the Loader2
...
describe('RobotCatalog — retry button on fetch error (resilience-matrix spec)', () => {
  ...
  it('shows the error message and a Reintentar button instead of an infinite spinner', () => {
  ...
  it('clicking Reintentar refetches the robot list', async () => {
    useRobotsMocks.error = new Error('Request timed out')
    render(<RobotCatalog />)
