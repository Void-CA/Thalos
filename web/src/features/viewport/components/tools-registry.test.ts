...
describe('TOOLS_BY_PERSPECTIVE — registry shape (design: accordion all-closed default)', () => {
  it('exposes the four robot tools (fk, ik, tcp, workspace-analysis) — workspace analysis joined the accordion (P0-B)', () => {
    expect(TOOLS_BY_PERSPECTIVE.robot.map((t) => t.id)).toEqual([
      'fk',
      'ik',
      'tcp',
      'workspace-analysis',
