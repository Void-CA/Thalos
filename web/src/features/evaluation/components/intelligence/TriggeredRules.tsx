...
export function TriggeredRules({ rules }: { rules: TriggeredRuleWire[] }) {
  return (
    <div className="flex flex-col gap-1.5">
      <h3 className="text-xs uppercase tracking-wider text-muted-foreground">Triggered Rules</h3>
      <p data-testid="assessment-rule-count" className="text-sm text-muted-foreground">
        {rules.length} rule{rules.length === 1 ? '' : 's'} triggered
      </p>
      <ul className="flex flex-wrap gap-2" data-testid="assessment-triggered-rules">
        {rules.map((rule) => (
          <li
            key={rule.id}
            data-testid="assessment-rule"
            className="rounded-md border border-border bg-secondary/10 px-2.5 py-1 text-xs font-mono"
          >
            {rule.id}
